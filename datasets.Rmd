...
from the immediate preceeding value of `ID`.  However, we always recommend using
unique `ID`.
...
otherwise.  mrgsolve also recoginzes dosing records where `ss=2`.  This allows
...
The column names in the preceeding list were written either as lower case form
...
sampling scheme or highly customised sampling schemes with very little work.
...
Occassionally, we want to simulate from a clinical data set (with observation
