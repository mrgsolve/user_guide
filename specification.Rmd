...
- `CXX11` to compile your model with C++11 standard; this adds
  the compiler flag `-std=c++11`
- `tad` to calculate time after dose inside your model code
- `N_CMT` to make available symbols that will let you access a compartment's 
  number by the name; so if `PERIPH` is the third compartment, you can 
  use `N_PERIPH` to reference that number
