...
The `mrgsolve` model specification involves several different types of code that
are written in different blocks.  A block starts with an identifier that takes
one of two forms: `$BLOCKID` or `[ BLOCKID ]`.  For example, parameters are
written in `$PARAM`, differential equations are written in `$ODE` etc.  Users
are free to include block code on the same line as the block identifier, but
must include a space after the identifier. For example, the parser will
...
adjacent time points to 0.25 time units, and request only the `RESP` compartment 
...
Preprocessor  `#define` directives are direct substitutions that the `C++` preprocessor makes prior to compiling your code.  
...
In the following section, we adopt the convention that `CMT` stands for a 
...
We want `CLi` to be a numeric value, so we use `double`.  To derived a `boolean` 
variable, write
```{c,eval=FALSE}
$MAIN
bool cure = false;
