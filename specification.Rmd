...
as the simulation run procees, we put the initialization of `x` in `$PREAMBLE`
...
error to include `$MAIN` or `$ODE` when `$PRED` is being used.  

See the section \@ref(pred-data) for additional information regardin data sets in use 
with `$PRED` block.  
...
### `TIME`
Gives the time in the curent data set record.  This is usually only used in 
